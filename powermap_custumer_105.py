...
if len(seuils_soc) <= 4:
    range_seuil_soc = 1
elif len(seuils_soc) <= 8:
    range_seuil_soc = 2
elif len(seuils_soc) <= 12:
    range_seuil_soc = 3
elif len(seuils_soc) <= 16:
    range_seuil_soc = 4
else:
    range_seuil_soc = 5

len_seuils_soc = len(seuils_soc)
for seuil_soc in range(range_seuil_soc):
    ...
    if int(len_seuils_soc/4) != 0:
        ...
        param_int = int('0b00' + bin(seuils_soc[4*seuil_soc+3])[2:].zfill(8) + bin(seuils_soc[4*seuil_soc+2])[
            2:].zfill(8) + bin(seuils_soc[4*seuil_soc+1])[2:].zfill(8) + bin(seuils_soc[4*seuil_soc])[
            2:].zfill(8), base=2)
    else:
        if (len(seuils_soc) % 4) == 2:
            seuils_soc_comment = f'Seuilx_x_{4*seuil_soc+1}_{4*seuil_soc}'
            param_int = int('0b00' + bin(seuils_soc[4*seuil_soc+1])[2:].zfill(8) + bin(seuils_soc[4*seuil_soc])[
                2:].zfill(8), base=2)
        elif (len(seuils_soc) % 4) == 3:
            seuils_soc_comment = f'Seuilx_{4*seuil_soc+2}_{4*seuil_soc+1}_{4*seuil_soc}'
            param_int = int('0b00' + bin(seuils_soc[4*seuil_soc+2])[
                2:].zfill(8) + bin(seuils_soc[4*seuil_soc+1])[2:].zfill(8) + bin(seuils_soc[4*seuil_soc])[
                2:].zfill(8), base=2)
        elif (len(seuils_soc) % 4) == 1:
            seuils_soc_comment = f'Seuilx_x_x_{4*seuil_soc}'
            param_int = int('0b00' + bin(seuils_soc[4*seuil_soc])[
                2:].zfill(8), base=2)
        else:
...
for seuil_T_idx in range(len(seuils_Temperature)):
    if seuils_Temperature[seuil_T_idx] < 0:
        seuils_Temperature[seuil_T_idx] += 256

if len(seuils_Temperature) <= 4:
    range_seuil_Temperature = 1
elif len(seuils_Temperature) <= 8:
    range_seuil_Temperature = 2
elif len(seuils_Temperature) <= 12:
    range_seuil_Temperature = 3
elif len(seuils_Temperature) <= 16:
    range_seuil_Temperature = 4
else:
    range_seuil_Temperature = 5

len_seuils_Temperature = len(seuils_Temperature)
for seuil_Temperature in range(range_seuil_Temperature):
    ...
    if int(len_seuils_Temperature/4) != 0:
        ...
        param_int = int('0b00' + bin(seuils_Temperature[4*seuil_Temperature+3])[2:].zfill(8) + bin(seuils_Temperature[4*seuil_Temperature+2])[
            2:].zfill(8) + bin(seuils_Temperature[4*seuil_Temperature+1])[2:].zfill(8) + bin(seuils_Temperature[4*seuil_Temperature])[
            2:].zfill(8), base=2)
    else:
        if (len(seuils_Temperature) % 4) == 2:
            seuils_Temperature_comment = f'Seuilx_x_{4*seuil_Temperature+1}_{4*seuil_Temperature}'
            param_int = int('0b00' + bin(seuils_Temperature[4*seuil_Temperature+1])[2:].zfill(8) + bin(seuils_Temperature[4*seuil_Temperature])[
                2:].zfill(8), base=2)
        elif (len(seuils_Temperature) % 4) == 3:
            seuils_Temperature_comment = f'Seuilx_{4*seuil_Temperature+2}_{4*seuil_Temperature+1}_{4*seuil_Temperature}'
            param_int = int('0b00' + bin(seuils_Temperature[4*seuil_Temperature+2])[
                2:].zfill(8) + bin(seuils_Temperature[4*seuil_Temperature+1])[2:].zfill(8) + bin(seuils_Temperature[4*seuil_Temperature])[
                2:].zfill(8), base=2)
        elif (len(seuils_Temperature) % 4) == 1:
            seuils_Temperature_comment = f'Seuilx_x_x_{4*seuil_Temperature}'
            param_int = int('0b00' + bin(seuils_Temperature[4*seuil_Temperature])[
                2:].zfill(8), base=2)
        else:
...
def update_threshold_values():
    ...
    for column in range(tablewidth):
        if column == 0:
            seuils_soc_labels[counter].config(text="0.." + str(entries_soc[column].get()))
        elif column == (tablewidth - 1):
            seuils_soc_labels[counter].config(text=str(entries_soc[column-1].get())+"..100")
        else:
            seuils_soc_labels[counter].config(text=str(entries_soc[column-1].get()) + ".." + str(entries_soc[column].get()))
        counter += 1
    ...
    for row in range(tableheight):
        if row == 0:
            if int(entries_T[row].get()) > 127:
                seuils_T_labels[counter].config(text="-20.." + str(int(entries_T[row].get()) - 256))
            else:
                seuils_T_labels[counter].config(text="-20.." + entries_T[row].get())
        elif row == (tableheight - 1):
            if int(entries_T[row-1].get()) > 127:
                seuils_T_labels[counter].config(text=str(int(entries_T[row-1].get()) - 256)+"..53")
            else:
                seuils_T_labels[counter].config(text=entries_T[row-1].get()+"..53")
        else:
            if (int(entries_T[row-1].get()) > 127) and (int(entries_T[row].get() > 127)):
                seuils_T_labels[counter].config(text=str(int(entries_T[row-1].get())-256) + ".." + str(int(entries_T[row].get())-256))
            elif ((int(entries_T[row-1].get()) <= 127) and (int(entries_T[row].get()) > 127)):
                seuils_T_labels[counter].config(text=str(entries_T[row-1].get()) + ".." + str(int(entries_T[row].get())-256))
            elif ((int(entries_T[row-1].get()) > 127) and (int(entries_T[row].get()) <= 127)):
                seuils_T_labels[counter].config(text=str(int(entries_T[row-1].get())-256) + ".." + entries_T[row].get())
            else:
                seuils_T_labels[counter].config(text=entries_T[row-1].get() + ".." + entries_T[row].get())

        counter += 1

# ---------------------------- Layout GUI ------------------------------- #

reset_button = Button(text="Reset Powermap to EVE", command=reset_powermap_to_EVE)
...
for column in range(len(seuils_Temperature)):
    ...
    if seuils_Temperature[column] > 127:
        ...
        entries_T[counter].insert(END, string=seuils_Temperature[column])
    entries_T[counter].grid(row=2 + math.floor(column/tablewidth), column=column % tablewidth + 1)
    counter += 1
...
for column in range(tablewidth):
    if column == 0:
        seuils_soc_labels[counter] = Label(text="0.." + str(seuils_soc[column]))
    elif column == (tablewidth - 1):
        seuils_soc_labels[counter] = Label(text=str(seuils_soc[column-1])+"..100")
    else:
        seuils_soc_labels[counter] = Label(text=str(seuils_soc[column-1]) + ".." + str(seuils_soc[column]))
...
for row in range(tableheight):
    if row == 0:
        if seuils_Temperature[row] > 127:
            seuils_T_labels[counter] = Label(text="-20.." + str(seuils_Temperature[row] - 256))
        else:
            seuils_T_labels[counter] = Label(text="-20.." + str(seuils_Temperature[row]))
    elif row == (tableheight - 1):
        if seuils_Temperature[row-1] > 127:
            seuils_T_labels[counter] = Label(text=str(seuils_Temperature[row-1] - 256)+"..53")
        else:
            seuils_T_labels[counter] = Label(text=str(seuils_Temperature[row-1])+"..53")
    else:
        if (seuils_Temperature[row-1] > 127) and (seuils_Temperature[row] > 127):
            seuils_T_labels[counter] = Label(text=str(seuils_Temperature[row-1]-256) + ".." + str(seuils_Temperature[row]-256))
        elif (seuils_Temperature[row-1] <= 127) and (seuils_Temperature[row] > 127):
            seuils_T_labels[counter] = Label(text=str(seuils_Temperature[row-1]) + ".." + str(seuils_Temperature[row]-256))
        elif (seuils_Temperature[row-1] > 127) and (seuils_Temperature[row] <= 127):
            seuils_T_labels[counter] = Label(text=str(seuils_Temperature[row-1]-256) + ".." + str(seuils_Temperature[row]))
        else:
            seuils_T_labels[counter] = Label(text=str(seuils_Temperature[row-1]) + ".." + str(seuils_Temperature[row]))
